...
int append_stub_and_set_entry(int fd, const char *signature, t_elf64_headers *hdrs)
{
	...
	memcpy(&stub[2],  &key,        sizeof(key));        // 4 bytes, custom value, loaded into ecx (will be overwritten below)
	memcpy(&stub[8],  &text_vaddr, sizeof(text_vaddr));  // 8 bytes, .text section base address, loaded into rcx
	memcpy(&stub[28], &sig_vaddr,  sizeof(sig_vaddr));   // 8 bytes, signature string address, loaded into rsi
	memcpy(&stub[37], &siglen32,   sizeof(siglen32));    // 4 bytes, signature length, loaded into edx
	memcpy(&stub[43], &text_len,   sizeof(text_len));    // 4 bytes, number of bytes in .text to decrypt
	memcpy(&stub[46], &orig_entry, sizeof(orig_entry));  // 8 bytes, original entry point, loaded into rax
...
int encrypt_text_section(int fd, Elf64_Shdr *text_shdr, uint32_t key)
{
	...
	if (pread(fd, buf, text_shdr->sh_size, text_shdr->sh_offset)
	...
	if (pwrite(fd, buf, text_shdr->sh_size, text_shdr->sh_offset)
		!= (ssize_t)text_shdr->sh_size)
	{
		free(buf);
		return 0;
